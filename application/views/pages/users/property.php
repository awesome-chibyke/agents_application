	 ...
	 <div class="row">
			...
			<div class="col-12">
                    <div style="width: 100%; text-align: left;" class="pull-left  wow fadeInUp">
                        <span class="pull-left"><h5>Properties <?php print $cat;?></h5></span>
                        
                         <span class="pull-right"> 
                         <span class="pull-right">
                         <?php  $cat_set = $cat_set;?>
                        	<select style="margin-bottom:5px !important;" class="form-control" name="goToType" id="goToType">
								<?php for($i=0;$i<count($category);$i++){?>
								<option <?php if(str_replace(" ","-",$category[$i]['property_category'])==str_replace(" ","-",$cat_set)) print 'selected="selected"';?> value="<?php echo str_replace(" ","-",$category[$i]['property_category']);?>"><?php echo $category[$i]['property_category'];?></option>
								<?php }?>
						   </select>
            ...
            <div class="is_card">A total of <?php print number_format($total_row);?> Properties <?php print $cat;?> in <?php print $siteInfo['siteName'];?>. Among these properties are houses, office, lands, shops, flats etc.We verify every property posted here you have nothing to be afraid of.</div>    						
				    ...
				    <div style="width: 100%; padding-top: 10px;">
						...
						<span  class="pull-right">
							...
							<?php if($property_data[$i]['whois_unique_id']==$user_data['unique_id']){}else{?>
							 | <span onClick="saveProperty('<?php print base_url();?>','<?php print $property_data[$i]['unique_id'];?>','<?php print $property_data[$i]['whois_unique_id'];?>','<?php print $user_data['unique_id'];?>');" class="saver fa fa-heart" title="Save Property"> Save</span>
							<?php }?>
